...
static void epoll_modify (EV_P_ int fd, int oev, int nev)
{
        ...
        if (expect_true(errno == ENOENT)) { /* if ENOENT then the fd went away, so try to do the right thing */
                if (!nev)
                        goto dec_egen;
                ...
                /*
                 * EPERM means the fd is always ready, but epoll is too snobbish to handle it,
                 * unlike select or poll.
                 */
...
static void epoll_poll(EV_P_ ev_tstamp timeout)
{
        ...
        for (i = 0; i < eventcnt; ++i) {
                ...
                if (expect_false((uint32_t)anfds[fd].egen != (uint32_t)(ev->data.u64 >> 32))) {
                        postfork |= 2; /* recreate kernel state */
                        continue;
                }
...
inline_size void epoll_fork(EV_P)
{
        close(backend_fd);
